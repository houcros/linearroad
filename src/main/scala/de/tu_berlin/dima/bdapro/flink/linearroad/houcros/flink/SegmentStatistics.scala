...
object SegmentStatistics {
  ...
  def numberOfVehicles(reports: DataStream[Tuple15[Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int]]): DataStream[(Int, Int, Int, Int)] ={
    ...
    reports
      .filter(_._1 == 0) // keep only position reports, no queries
  ...
  /**
    ...
    *         five minutes. Will emit every minute, except during the first 5 minutes.
    */
  def latestAverageVelocity(reports: DataStream[Tuple15[Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int,Int]]): DataStream[(Int, Int, Int, Float)] ={
    ...
    reports
      .filter(_._1 == 0) // only position reports
      ...
      .apply( (tup, timeWindow, iterable, collector: Collector[(Int, Int, Int, Float)]) => {
        ...
        // The avg that we want to calculate, for all vehicles in a location
        var grandAvg: Float = 0
        // How many different cars, for the grand average
        var counter: Int = 0
        // Per car, the accumulated average and the count, to calculate avg online
        val partialAvgs = mutable.Map[Int, (Float, Int)]()
        ...
        // As we accumulated all the avg speeds for each car, just need to divide per number of cars
        grandAvg /= counter
        val spl = iterable.head
